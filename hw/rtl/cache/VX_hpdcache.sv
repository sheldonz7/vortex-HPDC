...
`endif
    ...
    localparam CORE_REQ_DATAW = LINE_SIZE + TAG_WIDTH + FLAGS_WIDTH; //updated version of CORE_REQ_DATAWs
...
// hpcache
    ...
    // };
    // simplified version for vortex  
    localparam hpdcache_pkg::hpdcache_user_cfg_t HPDcacheUserCfg = '{
    nRequesters: NUM_REQS,
    paWidth: `CS_WORD_ADDR_WIDTH,
    wordWidth: WORD_SIZE * 8,
    sets: CACHE_SIZE / (LINE_SIZE * NUM_BANKS * NUM_WAYS),
    ways: NUM_WAYS,
    reqTransIdWidth: TAG_WIDTH,
    reqSrcIdWidth: `CS_REQ_SEL_BITS,
    ...
    .HPDcacheCfg(HPDcacheCfg)
        ...
        .core_rsp_ready_i(cache_rsp_ready),
        .mem_req_valid_o(mem_bus_if.req_valid),
        .mem_req_ready_i(mem_bus_if.req_ready),
        .mem_rsp_valid_i(mem_bus_if.rsp_valid),
        .mem_rsp_ready_o(mem_bus_if.rsp_ready)
    );
...
//simplifed version of the above code, Adjusted for compatibility with the modified core request signals
        VX_stream_xbar #(
        .NUM_INPUTS(NUM_REQS),
